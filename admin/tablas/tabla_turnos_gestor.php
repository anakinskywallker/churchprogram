...
$sql3= " SELECT EMP_ID, EMP_LUGAR FROM personal WHERE EMP_NOMBRE = '$nombre_usuario'";
    $result3=mysqli_query($conexion,$sql3);
    $var=mysqli_fetch_row($result3);
    $id_p = $var[0];
    $lugar_p = $var[1];
    
    $sql="SELECT TR_ID, CL_DOCUMENTO, TR_FECHA, TR_PAGO, TR_PRECIO, EMP_NOMBRE, EMP_APELLIDOS, GS_LUGAR_AUTORIZACION, GS_MUNICIPIO, CL_MUNICIPIO
FROM (
    SELECT T.TR_ID, C.CL_DOCUMENTO, T.TR_FECHA, T.TR_PAGO, T.TR_PRECIO, P.EMP_NOMBRE, P.EMP_APELLIDOS, TM.GS_LUGAR_AUTORIZACION, TM.GS_MUNICIPIO, C.CL_MUNICIPIO, ROW_NUMBER() OVER (PARTITION BY T.TR_ID ORDER BY T.TR_FECHA DESC) AS rn
    FROM turno T
    ...
    WHERE (TM.GS_ESTADO NOT IN (5, 6, 7, 8) AND ((TM.GS_ESTADO = 2 AND TM.GS_LUGAR_AUTORIZACION = '$var[1]') OR (TM.GS_ESTADO <> 2 AND TM.GS_MUNICIPIO = '$var[1]')))
) AS subquery
WHERE rn = 1
ORDER BY TR_FECHA ASC;
";
